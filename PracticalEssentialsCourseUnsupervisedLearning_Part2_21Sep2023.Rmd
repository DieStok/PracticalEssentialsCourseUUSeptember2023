...
# NOTE: I use a color-blind friendly palette, but for those who are not colour blind it might not be optimally distinct. You can change the palette if you want, as it is used throughout!
...
color_blind_friendly_colors = c("#a6cee3",'#1f78b4','#b2df8a','#33a02c')

##
## NOTE: if you are not colour blind you might like to change this palette to different values. See here: https://colorbrewer2.org/#type=qualitative&scheme=Dark2&n=4 
##
...
ggplot(data = dataforClustering, aes_string(x = "`Gene 1 Expression`", y = "`Gene 2 Expression`", colour = "`True Cluster`")) + geom_point(size = 3) +
  ...
  scale_colour_manual(values = color_blind_friendly_colors)
...
dataforClustering
#you start from that, and add noisy dimensions, do pca and clustering and plotting, etc.
...
Finally, let's have a look at UMAP. First, watch this short introduction to the idea of UMAP: https://www.youtube.com/watch?v=6BPl81wGGP8. Then watch the StatQuest for some more detail: https://www.youtube.com/watch?v=eN0wFzBA4Sc. Then scan through https://pair-code.github.io/understanding-umap/ for some more details. I trust you know the drill. Run UMAP and plot the results. Be sure to read a little bit of https://umap-learn.readthedocs.io/en/latest/ to understand the basic parameters. The function call is umap()
...
If you want to know even more UMAP details, see here: https://towardsdatascience.com/how-exactly-umap-works-13e3040e1668 .
